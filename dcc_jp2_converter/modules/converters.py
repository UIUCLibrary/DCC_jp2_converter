...
from dcc_jp2_converter import ImagemagickCommandBuilder, Exiv2CommandBuilder, KakaduCommandBuilder  # type: ignore
from dcc_jp2_converter import imagemagick_command_builders as im_cb                                 # type: ignore
from dcc_jp2_converter import kakaduCommandBuilders as kd_cb                                        # type: ignore
from dcc_jp2_converter import exiv2CommandBuilders as exi2_cb                                       # type: ignore
...
class Converter(metaclass=abc.ABCMeta):
    @staticmethod
    @abc.abstractmethod
    def convert_tiff_access_folder2(source: str, destination, overwrite_existing=True, remove_on_success=False):
        pass
...
class ImageMagickConverter(Converter):
    ...
    def convert_tiff_access_folder(path: str, overwrite_existing=True, remove_on_success=False):
        ...
        ImageMagickConverter.convert_tiff_access_folder2(path, path, overwrite_existing, remove_on_success)
    ...
    def convert_tiff_access_folder2(source: str, destination, overwrite_existing=True, remove_on_success=False):
        ...
        with tempfile.TemporaryDirectory(prefix="convert_") as tmp_folder:
            ...
            for i, tiff in enumerate(tiffs):
                ...
                logger.debug(
                    "Copying \"{}\" to temp folder \"{}\"".format(tiff, tmp_folder))
                ...
                logger.debug("Moving \"{}\" into \"{}\"".format(tmp_access_jp2, destination))
                if not os.path.exists(destination):
                    os.makedirs(destination)
                shutil.move(tmp_access_jp2, final_access_jp2)
                total_files_converted += 1
            logger.info("Created {} file(s) in {}.".format(total_files_converted, destination))
